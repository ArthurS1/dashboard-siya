...
const ChartsPage = () => {
  ...
  const [feedbacks, setFeedbacks] =
    useState<FeedbackData[] | undefined>(undefined);
  const [newletterAccounts, setNewsletterAccounts] =
    useState<NewsletterData[] | undefined>(undefined);
  const [apkDownloaded, setApkDownloaded] =
    useState<ApkDownloadsData[] | undefined>(undefined);
  const apply = () => {
  ...
  const newsletterAccountsData: DataPerDay[] | undefined =
    newletterAccounts?.reduce(
      (
        acc: DataPerDay[] | undefined,
        val: NewsletterData
      ): DataPerDay[] | undefined => {
      const currentValDate = new Date(val.date);
  ...
  const feedbacksData: DataPerDay[] | undefined = feedbacks?.reduce(
    (
      acc: DataPerDay[] | undefined,
      val: FeedbackData
    ): DataPerDay[] | undefined => {
